...
GLuint      g_location_tangentTexture;
GLuint      g_location_BiNormalTexture;

GLuint      location_counter;
...
void init( void )
{
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------------------------
    pTextureImage = auxDIBImageLoad( "tangent.bmp" );

	if( pTextureImage != NULL )
	{
		glGenTextures( 1, &tangentID );

		glBindTexture(GL_TEXTURE_2D, tangentID);
    ...
    //------------------------------------------------------------------------------------------------------------------------------------------------------------
    pTextureImage = auxDIBImageLoad( "biNormal.bmp" );

	if( pTextureImage != NULL )
	{
		glGenTextures( 1, &biNormal );

		glBindTexture(GL_TEXTURE_2D, biNormal);
...
void initShader( void )
{
    ...
    g_location_tangentTexture  = glGetUniformLocationARB( g_programObj, "TangentTexture" );	
    g_location_BiNormalTexture = glGetUniformLocationARB( g_programObj, "BiNormalTexture" );	
    location_counter           = glGetUniformLocationARB( g_programObj, "counter" );
...
void setShaderConstants( void )
{
	...
	///////////////////////////////////////////////////////////
        ...
        glUniform1iARB( g_location_normalTexture,   2);//g_textureID );	
        
        glUniform1iARB( g_location_tangentTexture,  3);//g_textureID );

        glUniform1iARB( g_location_BiNormalTexture, 4);//g_textureID );
	...
	// shader. However, we will do it this way now to avoid adding matrix code.
    	...
    	glUniform1fARB( location_counter, counter);				
	///////////////////////////////////////////////////////////
...
#include "longHorns_002.cpp"

GLfloat lightPos_marcTorso[] = {1.0, 1.0, 1.0, 1.0};
